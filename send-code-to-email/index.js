...
async function recoverEmailAndCodeIfAny(email, client) {
  try {
...
// TODO: This should be another email
...
functions.http("handler", async (req, res) => {
  ...
  try {
    ...
    const recovered = await recoverEmailAndCodeIfAny(
      validatedFields.email,
      knexClient
    );

    if (!recovered || recovered.length === 0) {
      console.log("Email not registered, exiting");
