...
async function updatePrompt(email, accesscode, prompt, client) {
  try {
    const result = await client("nftform")
      .update({ prompt: prompt })
      .where({ email: email, accesscode: accesscode });
    console.log("update prompt", result);
    return result;
  } catch (e) {
    console.error(e, e.stack);
    return null;
...
function validateFieldsFromRequest(parsedBody) {
  ...
  return {
    ...
    accesscode: validator.escape(parsedBody.accesscode),
...
functions.http("handler", async (req, res) => {
  ...
  try {
    ...
    const result = await updatePrompt(
      validatedFields.email,
      validatedFields.accesscode,
      validatedFields.prompt,
      knexClient
    );

    if (!result) {
      console.log("Failed to update, exiting");
